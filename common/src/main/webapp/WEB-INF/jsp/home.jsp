...
<body class="easyui-layout">
	...
	<div id="HomeFuncAccordion" class="easyui-accordion" style="width:213px;"
		...
		<div title="计划进度" data-options="selected:true" style="padding:10px;">
			<ul id="scheduleMonitor" class="easyui-tree" data-options="animate:true,lines:true">
				<li><span>计划进度</span>
					<ul>
						<li id=11 data-options="attributes:{'url':'schedule/order/find'}">订单管理</li>
					</ul>
					<ul>
						<li id=12 data-options="attributes:{'url':'schedule/custom/find'}">客户管理</li>
					</ul>
					<ul>
						<li id=13 data-options="attributes:{'url':'schedule/product/find'}">产品管理</li>
					</ul>
					<ul>
						<li id=14 data-options="attributes:{'url':'schedule/work/find'}">作业管理</li>
					</ul>
					<ul>
						<li id=15 data-options="attributes:{'url':'schedule/manufacture/find'}">生产计划管理</li>
					</ul>
					<ul>
						<li id=16 data-options="attributes:{'url':'schedule/task/find'}">生产派工管理</li>
					</ul>
				</li>
			</ul>
		</div>
		...
		<div title="工艺监控" data-options="selected:true" style="padding:10px">
			<ul id="technologyMonitor" class="easyui-tree" data-options="animate:true,lines:true">
				<li><span>工艺监控</span>
					<ul>
						<li id=31 data-options="attributes:{'url':'techonology/find'}">工艺管理</li>
						<li id=32 data-options="attributes:{'url':'techonology/technologyRequirement/find'}">工艺要求</li>
						<li id=33 data-options="attributes:{'url':'techonology/technologyPlan/find'}">工艺计划</li>
						<li id=34 data-options="attributes:{'url':'techonology/process/find'}">工序管理</li>
					</ul>
				</li>
			</ul>
		</div>
			...
			<ul id="materialMonitor" class="easyui-tree" data-options="animate:true,lines:true">
				<li><span>物料监控 </span>
					<ul>
						...
						<li id=42 data-options="attributes:{'url':'material/materialReceive/find'}">物料收入</li>
				    	<li id=43 data-options="attributes:{'url':'material/materialConsume/find'}">物料消耗</li>
					</ul>
				</li>
			</ul>
		</div>
		...
		<div title="质量监控" data-options="selected:true" style="padding:10px;">
			<ul id="qualifyMonitor" class="easyui-tree"
				...
				<li><span>质量监控</span>
					<ul>
						<li id=51 data-options="attributes:{'url':'quality/unqualify/find'}">不合格品管理</li>
						<li id=52 data-options="attributes:{'url':'quality/measure/find'}">成品计量质检</li>
						<li id=53 data-options="attributes:{'url':'quality/f_count_check/find'}">成品计数质检</li>
						<li id=54 data-options="attributes:{'url':'quality/p_measure_check/find'}">工序计量质检</li>
						<li id=55 data-options="attributes:{'url':'quality/p_count_check/find'}">工序计数质检</li>
					</ul>
				</li>
			</ul>
		...
		<div title="人员监控" data-options="selected:true" style="padding:10px">
			<ul id="employeeMonitor" class="easyui-tree"
				...
				<li><span>人员监控</span>
					<ul>
						<li id=61 data-options="attributes:{'url':'employee/department/find'}">部门管理</li>
					</ul>
					<ul>
		...
		<c:if test="${activeUser.rolename == '超级管理员' }">
			<div title="系统管理" style="padding:10px;">
				...
				<ul id="sysManager" class="easyui-tree"
					...
					<li><span>系统管理</span>
						<ul>
							<li data-options="attributes:{'url':'system/user/find'}">用户管理</li>
						</ul>
						<ul>
							<li data-options="attributes:{'url':'system/role/find'}">角色管理</li>
						</ul>
					</li>
				</ul>
			</div>
		...
		<c:if test="${activeUser.rolename == '超级管理员' }">
			<div title="申请管理" style="padding:10px;">
				<ul id="applyManager" class="easyui-tree"
					data-options="animate:true,lines:true">
					<li><span>申请管理</span>
						<ul>
							<li data-options="attributes:{'url':'apply/list/find'}">申请列表</li>
						</ul>
					</li>
				</ul>
			</div>
	...
	<script type="text/javascript">
		...
		$(function() {
				...
				onClick : function(node) {
					if ($('#scheduleMonitor').tree("isLeaf", node.target)) {
						...
						if (tab1) {
							...
							tabs1.tabs('add', {
								title : node.text,
								href : 'http://www.iscs.com:30003/'+ node.attributes.url,
								closable : true,
								bodyCls : "content"
							});
						}
			...
			$('#applyManager').tree({
				onClick : function(node) {
					if ($('#applyManager').tree("isLeaf", node.target)) {
						var tabs3 = $("#tabs");
						var tab3 = tabs3.tabs("getTab", node.text);
						if (tab3) {
							tabs3.tabs("select", node.text);
