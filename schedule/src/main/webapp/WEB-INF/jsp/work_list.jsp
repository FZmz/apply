...
<table class="easyui-datagrid" id="workList" title="作业列表" data-options="singleSelect:false,collapsible:true,
		pagination:true,rownumbers:true,url:'http://localhost:10010/schedule/work/list',method:'get',pageSize:10, fitColumns:true,toolbar:toolbar_work">
    <thead>
        <tr>
...
<div id="workEditWindow" class="easyui-window" title="编辑作业" data-options="modal:true,closed:true,resizable:true,
	iconCls:'icon-save',href:'http://localhost:10010/schedule/work/edit'" style="width:40%;height:55%;padding:10px;">
</div>
<div id="workAddWindow" class="easyui-window" title="添加作业" data-options="modal:true,closed:true,resizable:true,
	iconCls:'icon-save',href:'http://localhost:10010/schedule/work/add'" style="width:40%;height:55%;padding:10px;">
</div>
	        ...
	        <tr>
	            ...
	            <td>
	            	<input class="easyui-combobox" name="technologyPlanId" panelHeight="auto"  
    					...
    					url:'http://localhost:10010/techonology/technologyPlan/get_data',editable:false"/>
	            </td>
	        </tr>
	        <tr>
	            <td>工序顺序:</td>
	            ...
	            <td>
	            	<input id="deviceTypeName" class="easyui-combobox" name="deviceTypeId" panelHeight="auto"
    					data-options="required:true,editable:false,valueField:'deviceTypeId',textField:'deviceTypeName',
    						url:'http://localhost:10010/device/deviceType/get_data'" />
	            </td>
	        </tr>
	        <tr>
	            <td>设备状态:</td>
	            ...
	            <td>
		            <input id="deviceKeeper" class="easyui-combobox" name="deviceKeeperId" panelHeight="auto"
    					data-options="required:true,editable:false,valueField:'deviceKeeperId',textField:'deviceKeeper',
    						url:'http://localhost:10010/employee/get_data'" />
				</td>
	        </tr>
	        <tr>
	            <td>备注:</td>
...
function doSearch_work(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		$("#workList").datagrid({
			...
			nowrap:true, toolbar:"toolbar_work", url:'http://localhost:10010/schedule/work/list', method:'get', loadMsg:'数据加载中......',
		...
		$("#workList").datagrid({  
	        title:'作业列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get',
			nowrap:true, toolbar:"toolbar_work", url:'http://localhost:10010/schedule/work/search_work_by_'+name+'?searchValue='+value,
	...
	function  openWorkProduct(index){ 
		...
		$("#workProductInfo").dialog({
    		onOpen :function(){
    			$.get("http://localhost:10010/schedule/product/get/"+row.product.productId,'',function(data){
	...
	function submitWorkProductEditForm(){
		$.get("/product/edit_judge",'',function(data){
    		if(data.msg != null){
    			...
    			$.post("http://localhost:10010/schedule/product/update_all",$("#workProductEditForm").serialize(), function(data){
    				if(data.status == 200){
	...
	function  openWorkProcess(index){ 
		...
		$("#workProcessInfo").dialog({
			onOpen :function(){
				$.get("http://localhost:10010/schedule/process/get/"+row.process.processId,'',function(data){
		    		//回显数据
	...
	function submitWorkProcessForm(){
		...
		$.get("/process/edit_judge",'',function(data){
			if(data.msg != null){
				...
				//此处写工序信息
				$.post("http://localhost:10010/schedule/process/update_all",$("#workProcessForm").serialize(), function(data){
					if(data.status == 200){
	...
	function  openWorkDevice(index){ 
		...
		$("#workDeviceInfo").dialog({
    		onOpen :function(){
    			$.get("http://localhost:10010/schedule/deviceList/get/"+row.device.deviceId,'',function(data){
	...
	function submitWorkDeviceEditForm(){
		$.get("/deviceList/edit_judge",'',function(data){
    		if(data.msg != null){
    			...
    			$.post("http://localhost:10010/schedule/deviceList/update",$("#workDeviceEditForm").serialize(), function(data){
    				if(data.status == 200){
    	...
    	$.get("/work/delete_judge",'',function(data){
       		if(data.msg != null){
            	...
            	$.messager.confirm('确认','确定删除ID为 '+ids+' 的作业吗？',function(r){
            	    if (r){
            	    	var params = {"ids":ids};
                    	$.post("http://localhost:10010/schedule/work/delete_batch",params, function(data){
                			if(data.status == 200){
