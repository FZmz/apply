...
<table class="easyui-datagrid" id="manufactureList" title="生产计划列表" data-options="singleSelect:false,
		collapsible:true,pagination:true,rownumbers:true,url:'http://localhost:10010/schedule/manufacture/list',method:'get',pageSize:10,
...
<div id="manufactureEditWindow" class="easyui-window" title="编辑生产计划" data-options="modal:true,closed:true,
	resizable:true,iconCls:'icon-save',href:'http://localhost:10010/schedule/manufacture/edit'" style="width:40%;height:60%;padding:10px;">
</div>
<div id="manufactureAddWindow" class="easyui-window" title="添加生产计划" data-options="modal:true,closed:true,
	resizable:true,iconCls:'icon-save',href:'http://localhost:10010/schedule/manufacture/add'" style="width:40%;height:60%;padding:10px;">
</div>
	         ...
	         <tr>
	            ...
	            <td>
	            	<input id="custom" class="easyui-combobox" name="customId" data-options="required:true,
	            		valueField:'customId',textField:'customName',url:'http://localhost:10010/schedule/custom/get_data', editable:false" />
	            </td>
	        </tr>
	        <tr>
	            <td>订购产品:</td>
...
function doSearch_manufacture(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		...
		$("#manufactureList").datagrid({
			...
			nowrap:true, toolbar:"toolbar_manufacture", url:'http://localhost:10010/schedule/manufacture/list', method:'get', loadMsg:'数据加载中......',
		...
		$("#manufactureList").datagrid({  
	        title:'生产计划列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get',
			nowrap:true, toolbar:"toolbar_manufacture", url:'http://localhost:10010/schedule/manufacture/search_manufacture_by_'
	...
	function  openManuOrder(index){ 
		...
		$("#manuOrderInfo").dialog({
    		onOpen :function(){
    			$.get("http://localhost:10010/schedule/order/get/"+row.cOrder.orderId,'',function(data){
	...
	function submitManuOrderEditForm(){
		$.get("/order/edit_judge",'',function(data){
    		if(data.msg != null){
    			...
    			$.post("http://localhost:10010/schedule/order/update_all",$("#manuOrderEditForm").serialize(), function(data){
    				if(data.status == 200){
	...
	function  ManuTechnology(index){ 
		var row = onManufactureClickRow(index);
			$("#manuTechnologyInfo").dialog({
	    		onOpen :function(){
	    			$.get("http://localhost:10010/schedule/technology/get/"+row.technology.technologyId,'',function(data){
	 		    		//回显数据
	...
	function submitManuTechnologyEditForm(){
		$.get("/technology/edit_judge",'',function(data){
    		if(data.msg != null){
    			...
    			$.post("http://localhost:10010/schedule/technology/update_all",$("#manuTechnologyEditForm").serialize(), function(data){
    				if(data.status == 200){
    	...
    	$.get("/manufacture/delete_judge",'',function(data){
      		if(data.msg != null){
	        	...
	        	$.messager.confirm('确认','确定删除生产批号为 '+ids+' 的生产计划吗？',function(r){
	        	    if (r){
	        	    	var params = {"ids":ids};
	                	$.post("http://localhost:10010/schedule/manufacture/delete_batch",params, function(data){
	            			if(data.status == 200){
