...
<table class="easyui-datagrid" id="productList" title="产品列表" data-options="singleSelect:false,collapsible:true,
		pagination:true,rownumbers:true,url:'http://localhost:10010/schedule/product/list',method:'get',pageSize:20, fitColumns:true,
...
<div id="productEditWindow" class="easyui-window" title="编辑产品" data-options="modal:true,closed:true,resizable:true,
	iconCls:'icon-save',href:'http://localhost:10010/schedule/product/edit'" style="width:65%;height:80%;padding:10px;">
</div>
<div id="productAddWindow" class="easyui-window" title="添加产品" data-options="modal:true,closed:true,resizable:true,
	iconCls:'icon-save',href:'http://localhost:10010/schedule/product/add'" style="width:65%;height:80%;padding:10px;">
</div>
...
function doSearch_product(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		$("#productList").datagrid({
			...
			nowrap:true, toolbar:"toolbar_product", url:'http://localhost:10010/schedule/product/list', method:'get', loadMsg:'数据加载中......',
		...
		$("#productList").datagrid({  
	        title:'产品列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get',
			nowrap:true, toolbar:"toolbar_product", url:'http://localhost:10010/schedule/product/search_product_by_'+name+'?searchValue='+value,
	...
	function updateProductNote(){
		$.get("/product/edit_judge",'',function(data){
    		if(data.msg != null){
    			...
    			$.post("http://localhost:10010/schedule/product/update_note",$("#productNoteForm").serialize(), function(data){
    				if(data.status == 200){
    	...
    	$.get("/product/delete_judge",'',function(data){
       		if(data.msg != null){
            	...
            	$.messager.confirm('确认','确定删除ID为 '+ids+' 的产品吗？',function(r){
            	    if (r){
            	    	var params = {"ids":ids};
                    	$.post("http://localhost:10010/schedule/product/delete_batch",params, function(data){
                			if(data.status == 200){
