...
<table class="easyui-datagrid" id="orderList" title="订单列表" data-options="singleSelect:false,collapsible:true,
	pagination:true,rownumbers:true,url:'http://localhost:10010/schedule/order/list',method:'get',pageSize:10,fitColumns:true,toolbar:toolbar_order">
    <thead>
        <tr>
...
<div id="orderEditWindow" class="easyui-window" title="编辑订单" data-options="modal:true,closed:true,resizable:true,
	iconCls:'icon-save',href:'http://localhost:10010/schedule/order/edit'" style="width:65%;height:80%;padding:10px;">
</div>
<div id="orderAddWindow" class="easyui-window" title="添加订单" data-options="modal:true,closed:true,resizable:true,
	iconCls:'icon-save',href:'http://localhost:10010/schedule/order/add'" style="width:65%;height:80%;padding:10px;">
</div>
...
function doSearch_order(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		...
		$("#orderList").datagrid({
			...
			nowrap:true, toolbar:"toolbar_order", url:'http://localhost:10010/schedule/order/list', method:'get', loadMsg:'数据加载中......',
		...
		$("#orderList").datagrid({  
	        title:'订单列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get',
			nowrap:true, toolbar:"toolbar_order", url:'http://localhost:10010/schedule/order/search_order_by_'+name+'?searchValue='+value,
	...
	function  openOrderCustom(index){ 
		...
		$("#orderCustomInfo").dialog({
    		onOpen :function(){
    			$.get("http://localhost:10010/schedule/custom/get/"+row.custom.customId,'',function(data){
	...
	function submitOrderCustomEditForm(){
		$.get("http://localhost:10010/schedule/custom/edit_judge",'',function(data){
    		if(data.msg != null){
    			...
    			$.post("http://localhost:10010/schedule/custom/update_all",$("#orderCustomEditForm").serialize(), function(data){
    				if(data.status == 200){
	...
	function submitOrderProductEditForm(){
		$.get("http://localhost:10010/schedule/product/edit_judge",'',function(data){
    		if(data.msg != null){
    			$.messager.alert('提示', data.msg);
    		}else{
    			...
    			$.post("http://localhost:10010/schedule/product/update_all",$("#orderProductEditForm").serialize(), function(data){
    				if(data.status == 200){
	...
	function updateOrderNote(){
		$.get("http://localhost:10010/schedule/order/edit_judge",'',function(data){
    		if(data.msg != null){
    			$.messager.alert('提示', data.msg);
    		}else{
    			orderNoteEditor.sync();
    			$.post("http://localhost:10010/schedule/order/update_note",$("#orderNoteForm").serialize(), function(data){
    				if(data.status == 200){
    ...
    function order_edit(){
    	$.get("/schedule/order/edit_judge",'',function(data){
       		if(data.msg != null){
       			$.messager.alert('提示', data.msg);
       		}else{
    	...
    	$.get("/order/delete_judge",'',function(data){
      		if(data.msg != null){
              	...
              	$.messager.confirm('确认','确定删除ID为 '+ids+' 的订单吗？',function(r){
              	    if (r){
              	    	var params = {"ids":ids};
                      	$.post("http://localhost:10010/schedule/order/delete_batch",params, function(data){
                  			if(data.status == 200){
