...
<table class="easyui-datagrid" id="customList" title="客户列表" data-options="singleSelect:false,collapsible:true,
		pagination:true,rownumbers:true,url:'http://www.iscs.com:30003/schedule/custom/list', method:'get',pageSize:30, fitColumns:true,
...
<div id="customEditWindow" class="easyui-window" title="编辑客户" 
	data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save',href:'http://www.iscs.com:30003/schedule/custom/edit'"
	style="width:65%;height:80%;padding:10px;">
</div>

<div id="customAddWindow" class="easyui-window" title="添加客户" 
	data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save',href:'http://www.iscs.com:30003/schedule/custom/add'"
	style="width:65%;height:80%;padding:10px;">
</div>
...
function doSearch_custom(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		$("#customList").datagrid({
	        ...
	        toolbar:"toolbar_custom", url:'http://www.iscs.com:30003/schedule/custom/list', method:'get', loadMsg:'数据加载中......',
		...
		$("#customList").datagrid({  
	        title:'客户列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, 
	        	method:'get', nowrap:true,  
	        toolbar:"toolbar_custom", url:'http://www.iscs.com:30003/schedule/custom/search_custom_by_'+name+'?searchValue='+value,
	...
	function updateCustomNote(){
		$.get("/custom/edit_judge",'',function(data){
    		if(data.msg != null){
    			...
    			$.post("http://www.iscs.com:30003/schedule/custom/update_note",$("#customNoteForm").serialize(), function(data){
    				if(data.status == 200){
    	...
    	$.get("/custom/delete_judge",'',function(data){
       		if(data.msg != null){
    	    	...
    	    	$.messager.confirm('确认','确定删除ID为 '+ids+' 的客户吗？',function(r){
    	    	    if (r){
    	    	    	var params = {"ids":ids};
    	            	$.post("http://www.iscs.com:30003/schedule/custom/delete_batch",params, function(data){
    	        			if(data.status == 200){
    	        				$.messager.alert('提示','等待审核!',undefined,function(){
    	        					$("#customList").datagrid("reload");
    	        				});
    	        			}
    	        		});
